...
export class Input2Component implements OnInit {
  ...
  constructor(private router: Router,private afAuth: AngularFireAuth,public  db: AngularFireDatabase) { 
    this.careers = this.db.list('users/users/'+this.afAuth.auth.currentUser.uid+'/measure/data/area/area').valueChanges();
  ...
  //   ,n6:number,n7:number,n8:number,n9:number,n10:number,nn:number,esc:string,measure:number,vio:string,date:string
